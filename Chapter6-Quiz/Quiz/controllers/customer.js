    ...
    customerBeli : async (req, res, next) => {
        try {
            ...
            const customerTransaksi = response.map(customer => {
                const totalPembelian = customer.transaction.reduce((sum, transaction) => sum + transaction.total_pembelian, 0);
                return {
    ...
    customerTop : async (req, res, next) => {
        try {
            ...
            const customerTotals = response.map(customer => {
                const totalPembelian = customer.transaction.reduce((sum, transaction) => sum + transaction.total_pembelian, 0);
                const totalItem = customer.transaction.reduce((sum, transaction) => 
                    sum + transaction.detail_transaction.reduce((detailSum, detail) => detailSum + ((detail.id_product - (detail.id_product-1))*detail.jumlah), 0), 0);
                return {
                  name: customer.nama_customer,
