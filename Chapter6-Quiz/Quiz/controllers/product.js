...
module.exports = {
    ...
    productFavorit : async (req, res, next) => {
        try {
            ...
            const dataProduk = response.map(product => {
                const jumlahFrekuensi = product.detail_transaction.length;
                const totalPembelian = product.detail_transaction.reduce((sum, detail) => sum +  (detail.jumlah*product.harga), 0);
    ...
    productKurangLima : async (req, res, next) => {
        try {
            ...
            const produkDipilih = response.filter(product => product.detail_transaction.length > 0);
